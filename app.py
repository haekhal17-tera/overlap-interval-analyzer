...
def load_intervals_from_csv(file_or_path, start_col: str, end_col: str, max_hours: float):
    """
    Load interval [start, end] dari CSV trip records
    """
...
st.markdown(
    """
Aplikasi ini menganalisis **kompleksitas dan running time algoritma deteksi interval overlap**
dengan dua pendekatan:

- **Iteratif (Sweep Line)** → O(n)
- **Rekursif (Divide & Conquer)** → O(n log n)

Dataset yang digunakan adalah **data trip nyata (`taxi_tripdata.csv`)**.  
Pengguna **mengatur ukuran input (n) secara manual** untuk eksperimen kompleksitas.
"""
)
...
with st.expander("Definisi overlap (untuk laporan)", expanded=False):
    st.markdown(
        """
Interval dianggap **overlap** jika setelah diurutkan berdasarkan waktu mulai (`start`),
berlaku:
...
algorithm_choice = st.sidebar.radio(
    "Metode Deteksi",
    ["Iteratif (Sweep) — O(n)", "Rekursif (Divide & Conquer) — O(n log n)"]
)

detect_fn = (
    count_overlaps_iterative_sorted
    if "Iteratif" in algorithm_choice
    else count_overlaps_recursive_divconq_sorted
)

repeat = st.sidebar.number_input(
    "Repeat runtime (ambil median)",
...
if data_source.startswith("File lokal"):
    if os.path.exists(DATASET_FILENAME):
        ...
        st.error(
            ...
            "Letakkan file tersebut di root project."
        )
else:
...
intervals, df_display = load_intervals_from_csv(
    csv_obj, start_col, end_col, max_hours
)
...
if st.button("Jalankan Benchmark"):
    try:
        ...
        for n in sizes:
            ...
            t_iter = median_runtime_ms(
                count_overlaps_iterative_sorted, subset, repeat=repeat_bench
            )
            t_rec = median_runtime_ms(
                count_overlaps_recursive_divconq_sorted, subset, repeat=repeat_bench
            )

            rows.append((n, t_iter, t_rec))

        df_result = pd.DataFrame(rows, columns=["n", "iter_ms", "rec_ms"])
