...
def main():
    ...
    intervals = load_intervals(args.csv, args.start_col, args.end_col, max_hours=args.max_hours)
    print("Total intervals after clean:", len(intervals))
    ...
    for n in sizes:
        ...
        iter_overlap = count_overlaps_iterative(subset)
        rec_overlap = count_overlaps_recursive_divconq(subset)

        iter_ms = median_runtime_ms(count_overlaps_iterative, subset, repeat=args.repeat)
        rec_ms = median_runtime_ms(count_overlaps_recursive_divconq, subset, repeat=args.repeat)
